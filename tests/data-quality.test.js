...
test('定期計算日程を誤り・未確認として集計しない', () => {
  const report = buildDataQuality([{
    ...
    meetings: [
      ...
      { date_basis: 'recurrence_calculated', next_date: '2026-10-02' },
      { date_basis: 'manual_date', next_date: '2026-10-03' },
    ],
...
test('公開品質JSONは全都道府県と全例会を集計している', () => {
  ...
  assert.equal(
    report.summary.official_date + report.summary.manual_date + report.summary.recurrence_calculated
