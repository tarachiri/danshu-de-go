...
### 8. コード・ドキュメントのコメントは「他のAIが読む前提」で書く

かもちゃん・ふーちゃん・チャッピー・断かも等、複数のAIが同じコードベースを
入れ替わり読み書きするため、コメントは「未来の別セッションの自分」または
「別のAI」が読んだときに誤解しないことを目的に書く。

**明記すべきこと：**

- **確認済みか推測かを区別する**
  IPアドレス・ポート番号・ファイルパス・接続方式などは、
  実測して確認したのか、ドキュメントからの推測なのかを一言添える。
  ```python
  # 確認済み: 2026-08-02、tyoで`tailscale ip -4`実行し100.101.127.28と実測
  TYO_IP = "100.101.127.28"
  ```
  「Tailscale経由でアクセス」のような曖昧な表現だけを残すと、
  別ホストのファイルパスを直接開けると誤解されるなど、
  実装ミスの原因になる（2026-08-02掲示板投稿障害の実例）。

- **過去の失敗・設計変更の理由を残す**
  「なぜ今の実装になったか」を、変更前の状態と合わせて書く。
  修正して終わりにせず、同じ地雷を踏み直さないための注記として残す。
  ```python
  # 注意: 過去(2026-08-01)は sqlite3.connect("/home/maji/danshu.db") を
  # genから直接呼び出す実装だったが、これはgen上に空DBを誤作成し
  # 投稿失敗の原因になった。現在はtyoのAPI(100.101.127.28:8769)へ
  # HTTPプロキシする方式。詳細: docs/notes/2026-08-02-bulletin-tyo-api.md
  ```

- **他マシン・他プロセスとの通信箇所は、実際の経路を明記する**
  IP・ポート・認証方式（共有キー等）を、伏字にすべき値を除いて明記する。

**ファイル名の自己記載（新ルール）**

作業記録・指示書等の.mdファイルは、先頭または末尾に、そのファイル自身の
ファイル名を明記すること。

```markdown
<!-- ファイル名: 2026-08-02-bulletin-tyo-api.md -->
```

理由：Google Driveやチャット添付など、ファイルが複製・転送された際に
ファイル名の情報が失われる場合がある。ファイル名が本文内にあれば、
どの作業記録かを本文だけで判別でき、AI間での参照・引用時の取り違えを防げる。
...
## .mdファイル作成時のルール：「要約＆感想」ブロックの同梱

作業記録の.mdファイルを新規作成する際は、本文（技術ログ）の**冒頭**に、
以下の形式で「要約＆感想」ブロックを必ず追加すること。

```
【要約＆感想】
- 何をしたか：（利用者目線で1〜2行、専門用語を避けて）
- なぜ嬉しいか：（断酒会利用者・支援者にとっての意味を1〜2行）
- ひとこと感想：（自由に、自然な言葉で）
```

注意点：
- 依存症・アルコール依存症・例会・回復・支援・断酒等の言葉を、
  不自然でない範囲で自然に含めること（キーワードの無理な詰め込みは避ける）
- コミットハッシュ・ファイルパス・スクリプト名・DB列名等の技術的固有名詞は
  このブロックに書かないこと（本文側に残す）
- このブロックは将来的にブログ日記生成（gemma4）の元ネタとして
  自動的に読み取られる。技術ログとしての本文の正確性・詳細さは
  従来通り維持し、このブロックの追加によって本文を省略・簡略化しないこと
